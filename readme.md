...
script/filter-meaningful-diff.js は、本文・タイトル・画像・リンクが実際に変わったファイルだけを抽出する。広告タグやスクリプトの細かい変更だけで、AI校正や最終更新日の書き換えが無駄に走らないようにするため。
なお、記事一覧やサイトマップの生成はこの結果を使わず、毎回すべての記事をスキャンして作り直す。
